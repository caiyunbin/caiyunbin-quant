// 共用导航栏 — 所有需要登录保护的页面调用 renderNav(isSubdir)
// isSubdir: 子目录(strategies/*.html) 传 true, 根目录传 false

function renderNav(isSubdir) {
...
<aside id="mobile-menu" class="mobile-menu">
    <div class="p-5 border-b">
        ...
        <div class="text-xs text-gray-500 mt-1" id="m-auth-info">已登录</div>
    </div>
    <nav class="p-3 space-y-1">
        <a href="${prefix}index.html" class="block px-4 py-3 rounded text-gray-700 hover:bg-indigo-50 hover:text-indigo-600">📊 总览</a>
        ...
        <a href="${prefix}strategies/etf.html"           class="block px-4 py-3 rounded text-gray-700 hover:bg-indigo-50">🏦 板块轮动</a>
    ...
    if (info && info.phone) {
        document.getElementById('m-auth-info').textContent = '已登录 ' + info.phone;
    }
}
...
<footer class="bg-white border-t border-gray-200 mt-12">
    <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 text-center">
        <p class="text-xs text-gray-400">
            ⚠ 风险提示: 本网站展示内容仅供本人量化研究参考, 不构成任何投资建议或收益承诺. 证券市场有风险, 投资需谨慎.
        </p>
